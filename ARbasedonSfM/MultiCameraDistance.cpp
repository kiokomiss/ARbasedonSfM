...
features_matched(false),use_rich_features(true)
{		
	...
	//load calibration matrix if exist -TODO!!
	//Create mockup camera matrix
...
#pragma omp parallel for
		for (frame_num_i = 0; frame_num_i < loop1_top; frame_num_i++) {
			...
			{
				std::cout << "------------ Match " << frame_num_i << ","<<frame_num_j-1<<" ------------\n";
		...
		typedef std::map<std::pair<int, int>, std::vector<cv::DMatch> >::const_iterator MapIterator;
		int count_match = 0;
		for (MapIterator it = matches_matrix.begin(); it != matches_matrix.end(); it++) {
