...
void MultiCameraPnP::GetBaseLineTriangulation() {
	...
	cout << "matches_size = ";
	for (list<pair<int, pair<int, int> > >::iterator i = matches_sizes.begin(); i != matches_sizes.end(); ++i) {
		...
		++highest_pair) 
	{
		...
		std::cout << " -------- " << m_first_view << " and " << m_second_view << " -------- " <<std::endl;
		...
		if (goodF) {
			...
			if(!good_triangulation || cv::countNonZero(add_to_cloud) < 10) {
				std::cout << "good_triangulation=" << good_triangulation << ",cv::countNonZero(add_to_cloud)= " << cv::countNonZero(add_to_cloud) << std::endl;
	...
	cout << "Taking baseline from " << m_first_view << " and " << m_second_view << endl;
	...
	) 
{
	...
	vector<int> inliers;
		...
		CV_PROFILE("solvePnPRansac",cv::solvePnPRansac(ppcloud, imgPoints, K, distortion_coeff, rvec, t, true, 1000, 0.006 * maxVal, min(0.25, 0.25 * (double)(imgPoints.size())), inliers, CV_EPNP);)
